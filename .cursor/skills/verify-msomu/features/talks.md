...
The `/talks` index lists presentations from the `talks` content collection. Slide decks are static Reveal.js HTML under `/talks/<slug>` (Workers static assets serve `public/talks/`). With `trailingSlash: "never"`, the canonical deck URL has no trailing slash.
...
- `@astrojs/cloudflare` 14 is Workers-only (no `routes.exclude` / `_routes.json`). `public/talks/` is served as static assets. `scripts/dedupe-talks-routes.mjs` no-ops when `_routes.json` is absent.
- `trailingSlash: "never"` makes `/talks/` and `/talks/receipt/` 404 on the Astro dev server. Drive the slash-free paths.
